...
# ---------------------------------------------------------------- scene -------
def load_scene():
    top = Image.open(hf_hub_download(VLA_REPO, "assets/sample_realsense_top_rgb.png")).convert(
        "RGB"
    )
    side = Image.open(hf_hub_download(VLA_REPO, "assets/sample_realsense_side_rgb.png")).convert(
        "RGB"
    )
...
def act(top, side, task, state, num_steps: int = 10) -> dict:
    ...
    use_transformers(
        ...
        parameters={
            "pretrained_model_name_or_path": VLA_REPO,
